...
void Task_Main() {
	...
	*(myResult.resultCount) = 0;
	...
	while (isRunning) {
		...
		if (*(myResult.resultCount) == TASKCOUNT)
		{
			printf("[Main] --FULL-- Task/Result/Total:%d/%d/%d\n", *(myTask.taskCount), *(myResult.resultCount), TASKCOUNT);
			//重新分配任务
			*(myTask.taskCount) = TASKCOUNT;
			*(myResult.resultCount) = 0;

			for (size_t j = 0; j < MAX_THREADS; j++)
			{
				thread = threadArray + j;
				thread->toThreadMessage.type = 11;
				printf("[Main]:[%d] toThreadMessage.type = 11\n", thread->id);
			}
