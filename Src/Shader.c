	...
	*(_getRenderer()->taskIndex) = 0;
	while (true)
	{
		...
		case 2:
			...
			LeaveCriticalSection(_getRenderer()->vertexIndexLock);
			if (currentTriangleIndex >= _getGameIns()->cMesh->geo.numOfQuad * 2)
			{
				//切换渲染阶段
				r->renderStage = 3;
				//重置任务数
			...
			*(_getGameIns()->cMesh->triangleBBox + currentTriangleIndex + 0) = x_min;
			*(_getGameIns()->cMesh->triangleBBox + currentTriangleIndex + 1) = y_min;
			*(_getGameIns()->cMesh->triangleBBox + currentTriangleIndex + 2) = x_max;
			*(_getGameIns()->cMesh->triangleBBox + currentTriangleIndex + 3) = y_max;
